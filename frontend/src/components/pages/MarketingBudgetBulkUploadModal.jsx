...
export async function downloadMarketingBudgetTemplate({
  ...
  noteCell.value = 'Petunjuk: Kolom bertanda (*) wajib diisi. Nama Akun CoA harus sesuai daftar di tab "Referensi_CoA".';
  ...
  const headers = [
    ...
    { key: 'coa_account', header: 'Akun CoA *', width: 36 },
    { key: 'vendor_name', header: 'Vendor Partner', width: 30 },
  ...
  headers.forEach((h, i) => {
    ...
    cell.alignment = { vertical: 'middle', horizontal: i === 0 || i === 4 ? 'center' : i === 5 ? 'right' : 'left' };
    cell.border = {
  ...
  if (includeCurrentData && currentItems.length > 0) {
    ...
    // Sample rows
    const sampleRows = [
      [1, 'Januari', metadata.coas?.[0]?.name || 'Advertising & Promotion Event', 'PT Media Kreasi Nusantara', 1, 15000000, 'Billboard & OOH Placement Senayan'],
      [2, 'Januari', metadata.coas?.[1]?.name || 'Documentation', 'Studio Visual Prima', 1, 4500000, 'Dokumentasi foto & video event'],
      [3, 'Februari', metadata.coas?.[2]?.name || 'Cost Of Event', 'Vendor Dekorasi Indonesia', 1, 12000000, 'Setup booth display & sound system'],
      [4, 'Maret', metadata.coas?.[3]?.name || 'Selling Expenses', '', 10, 500000, 'Voucher promosi loyalty customer']
    ];
  ...
  coaSheet.addRow(['Daftar Akun CoA Resmi']).font = { bold: true, size: 11, color: { argb: 'FF1E40AF' } };
  coaSheet.addRow(['Gunakan nama akun dari tabel ini untuk kolom "Akun CoA *"']).font = { italic: true, size: 9, color: { argb: 'FF64748B' } };
  coaSheet.addRow([]);

  const coaHeaderRow = coaSheet.addRow(['Kode Akun', 'Nama Akun CoA']);
  coaHeaderRow.font = { bold: true, color: { argb: 'FFFFFFFF' } };
  coaHeaderRow.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF2563EB' } };
  coaSheet.getColumn(1).width = 18;
  coaSheet.getColumn(2).width = 42;

  if (metadata.coas && metadata.coas.length > 0) {
    metadata.coas.forEach(c => {
      coaSheet.addRow([c.code || '-', c.name]);
    });
  }
  ...
  if (metadata.vendors && metadata.vendors.length > 0) {
    ...
    vendorSheet.addRow(['Daftar Vendor Partner']).font = { bold: true, size: 11, color: { argb: 'FF1E40AF' } };
    vendorSheet.addRow(['(Opsional) Anda juga dapat mengisi nama vendor rekanan baru']).font = { italic: true, size: 9, color: { argb: 'FF64748B' } };
    vendorSheet.addRow([]);

    const vHeader = vendorSheet.addRow(['Kode Vendor', 'Nama Vendor']);
  ...
  const processFile = async (selectedFile) => {
    ...
    try {
      ...
      for (let r = 1; r <= 10; r++) {
        ...
        if (joined.includes('bulan') || joined.includes('month') || joined.includes('coa') || joined.includes('akun')) {
          headerRowNumber = r;
          ...
          row.eachCell((cell, colNumber) => {
            ...
            else if (val.includes('coa') || val.includes('akun') || val.includes('account')) colMap.coa = colNumber;
      ...
      sheet.eachRow((row, rowNumber) => {
        ...
        if (rawCoa) {
          ...
          matchedCoa = metadata.coas?.find(c => c.name.trim().toLowerCase() === searchCoa) ||
                       metadata.coas?.find(c => c.code && c.code.trim().toLowerCase() === searchCoa) ||
                       metadata.coas?.find(c => c.name.trim().toLowerCase().includes(searchCoa));
        }

        if (!matchedCoa) {
          errors.push(`Akun CoA "${rawCoa || '-'}" tidak dikenali`);
        }
        ...
        if (rawVendor) {
          ...
          const matchedVendor = metadata.vendors?.find(v => v.vendor_name.trim().toLowerCase() === searchVendor.toLowerCase()) ||
                                metadata.vendors?.find(v => v.vendor_code && v.vendor_code.trim().toLowerCase() === searchVendor.toLowerCase());
          if (matchedVendor) {
                      ...
                      <table className="w-full text-left text-[10px] border-collapse table-fixed">
                        <thead>
                          <tr className="bg-neutral-50 dark:bg-neutral-950 border-b border-neutral-200 dark:border-neutral-800 text-neutral-400 font-extrabold uppercase tracking-wider sticky top-0 z-10">
                            ...
                            <th className="px-2 py-2 w-44">Akun CoA</th>
                        ...
                        <tbody className="divide-y divide-neutral-100 dark:divide-neutral-800 text-neutral-700 dark:text-neutral-300 font-medium">
                          {parsedRows.map((r, idx) => (
                            ...
                            >
                              ...
                              <td className="px-2 py-1.5 truncate text-neutral-500 dark:text-neutral-400" title={r.vendor_name}>
                                {r.vendor_name || '-'}
                              </td>
