...
export default function LegalDocPage({ config }) {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await apiClient.get('/api/legal/legal-docs', {
        params: {
          module,
          page,
          limit: 20,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingDoc) {
        await apiClient.put(`/api/legal/legal-docs/${editingDoc.id}`, payload);
      } else {
        await apiClient.post('/api/legal/legal-docs', payload);
      }
  ...
  const handleViewDetail = async (doc) => {
    ...
    try {
      const full = await apiClient.get(`/api/legal/legal-docs/${doc.id}`);
      setSelectedDoc(full);
    } catch (err) {
  ...
  const confirmDeleteDoc = async () => {
    ...
    try {
      setSubmitting(true);
      await apiClient.delete(`/api/legal/legal-docs/${docToDelete.id}`);
      fetchData();
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await Promise.all(selectedIds.map(id => apiClient.delete(`/api/legal/legal-docs/${id}`)));
      setSelectedIds([]);
      fetchData();
    } catch (err) {
