...
// Helper function to generate suggested Vendor Code (e.g. PT CENTRIN ONLINE PRIMA -> VND-CENTRIN)
const generateVendorCode = (name) => {
  if (!name) return '';
  const words = name.split(/\s+/).map(word => word.replace(/[.,\/#!$%\^&\*;:{}=\-_`~()]/g, '').toUpperCase());
  const cleanWords = words.filter(w => w !== 'PT' && w !== 'CV' && w !== 'UD' && w !== '');
  if (cleanWords.length === 0) return 'VND-';
  return `VND-${cleanWords[0]}`;
};
  ...
  const formatIDR = (val) => {
    ...
    <div className="space-y-6 relative pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button
          onClick={() => { setEditingVendor(null); setFormData({ ...defaultFormData }); setIsCodeAutoGenerated(true); setShowAddDrawer(true); }}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        const updatedFields = { vendor_name: newName };
                        if (isCodeAutoGenerated) {
                          updatedFields.vendor_code = generateVendorCode(newName);
                        }
                        setFormData({...formData, ...updatedFields});
                      }}
                      onBlur={(e) => {
                        const formatted = formatVendorName(e.target.value);
                        const updatedFields = { vendor_name: formatted };
                        if (isCodeAutoGenerated) {
                          updatedFields.vendor_code = generateVendorCode(formatted);
                        }
                        setFormData({...formData, ...updatedFields});
                      }}
